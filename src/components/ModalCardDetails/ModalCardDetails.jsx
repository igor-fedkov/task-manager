...
import { ListActivity, CommentsList } from '../';
...
function ModalCardDetails({isModalShow, onModalClose}) {
  ...
  const onRemoveCard = useCallback(() => {
    ...
    dispatch(cardsOperations.deleteCard(dataForDel));
    onModalClose();
  ...
  const onAddDescription = useCallback(() => {
    const newCard = { ...card, owner: userId};
    newCard.description = inputDescription.current.value.trim();
    inputDescription.current.value = '';
    dispatch(cardsOperations.addDescription(newCard));
    setIsDesctiptionEdit(false);
  }, [card, userId, dispatch]);
  ...
  const onAddComment = useCallback(() => {
    ...
    dispatch(commentsOperations.addComment(comment));
    inputComment.current.value = '';
    setIsBtnSaveCommentDisabled(true);
  }, [card?.id, userId, dispatch]);
